...
\item
  ...
  \item Based on Table 8.3 in the text, we have that this process capability ratio does not meet the minimum recommended level of 1.33. While the process may be capable of meeting specifications, it doesn't seem likely that it would happen as consistently as desired.
  ...
  \item If the manufacturer seeks to increase the $C_p$, they must first, make sure the center is lowered from its current value of $\mu = 25$ to $\mu_1 = 23.5 + \frac{26.1-23.5}{2} = \Sexpr{23.5 + (26.1-23.5)/2}$ so that the process is centered. Also, the characteristic standard deviation $\sigma = 1.25$ is far too large for the process to be considered capable of meeting specifications.
    
  \end{enumerate}

\item 
  \begin{enumerate}
    \item We have that $\hat{\sigma} = \frac{\bar{R}}{d_2} = \Sexpr{.8865 * 1.9}$.
    \item We have that 
      \begin{align*}
    ...
    \item I would not suggest adopting these trial control limits if the machine is considered to be meeting some desired specification as there is one out of control signal in the $I$-chart labeled ``Test''. I would also want to investigate into possible explinations for such a high observation.
      
    \item The difference between the last and second to last observation is greater than the average absolute difference between observations ($\bar{R} = 1.9$) so removal of the last observation would slightly decrease the $CL$ for the $MR$ chart. The $LCL$ would remain 0 and the $UCL$ would decrease slightly as it is a linear function of $\bar{R}$ (namely, $UCL = D_4 \bar{R}$).
      
    \item Because the last observation is slightly above the sample average of $\bar{x} = 150.37$, we have that the centerline would decrease for the $I$ chart. The $UCL$ and $LCL$ are positive and negative linear functions of $\bar{R}$, respectively. Removing the last observation causes a decrease in $\bar{R}$ and thus a decrease and increase in the $UCL$ and $LCL$ for the $I$ chart.
  \end{enumerate}
  ...
  \item
    ...
    \item It appears that samples 1, 44-45, 47, 49, 52-56, 65, 67-68, 70-71, are above aim and samples  21-27, 29-30, 60, and 73-80 are all below aim. All other samples appear to be on target (within the control limits).
  ...
  \item 
    ...
    \item For the data used in plot 1, we have
      ...
      These are both small $\hat{C}_p$ values and do not suggest that the process can consistently product wafers within specifications.
  ...
  \item 
    ...
    \item Now, there appears to be symmetry in the data and the process is not yielding any out of control signals.
      \begin{center} 
        \includegraphics[width=.75\linewidth]{q8c.png}
      \end{center}
      
    \item I would recommend that they use the log transformed data as that appears more symmetric and is better modeled using a normal distribution. This is an important assumption made in several of the quality control analysis techniques.
