...
\begin{enumerate}
  \item 
    \begin{enumerate}
      \item Histogram of density. The density appears to be right skewed but other wise have an approximate normal distribution:
  ...
  \item 
    \begin{enumerate}
      \item \textbf{Rule 1:} Not sure what you want here: The probability of finding at least one in seven sample means to be beyond three $\sigma$ deviations from the mean when the process is actually in control. This is 1 minus the probability of finding all seven sample means to be within 3 $\sigma$ of the mean. We know the sample means will be distributed normally with standard deviation $\sigma/\sqrt{5}$.
        
      \item \textbf{Rule 2:} The probability of finding all seven sample means to be either all above or below the mean quality characteristic. Assuming the characteristic is independently and symmetrically distributed, then this would simply be $\alpha = (\frac{1}{2})^7$.
  ...
  \item
    \begin{enumerate}
      ...
      \item $$ P = 1 - P^*_0 - P^*_1 $$
        ...
        <<thishere, echo=F>>=
        b = pnorm(2)-pnorm(1)+pnorm(-3)-pnorm(-4)
        @
    ...
    \item The population mean and standard deviation would have to be estimated with the data instead of being provided.
      
    \item Pareto charts:
      <<q6a, echo=F, out.width='.7\\linewidth', fig.width=10, fig.height=7>>=
      ...
      week1$cause = with(week1, factor(week1$cause, levels=week1[order(-totals),]$cause))
      ggplot(week1, aes(cause, totals)) + geom_bar(stat='identity')
@
